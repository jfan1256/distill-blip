...
def eval(model, data_loader, device, batch_size):
    ...
    for i, (image, caption, idx) in tqdm(enumerate(data_loader), desc="Evaluation"):
        ...
        gen_caption = model.generate(image, sample=True)

        for id, dlip_cap, org_cap in zip(idx, gen_caption, caption):
...
def main(args, config):
    ...
    for i in range(len(evaluate)):
        gen_caption = evaluate.iloc[i]['dlip_caption']
        original_caption = evaluate.iloc[i]['original_caption']
        print(f"Row {i + 1:<3}: DLIP Caption: \"{gen_caption:<80}\", Original Caption: \"{original_caption}\"")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='../configs/dlip_caption_flickr_eval.yaml')
    parser.add_argument('--output_dir', default='output/Caption_dlip_eval')
