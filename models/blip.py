...
class BLIP_Base(nn.Module):
    def __init__(self,
                 med_config='configs/bert-base_config.json',
                 image_size=224,
                 vit='base',
                 vit_grad_ckpt=False,
                 vit_ckpt_layer=0,
...
def create_vit(vit, image_size, use_grad_checkpointing=False, ckpt_layer=0, drop_path_rate=0):
    assert vit in ['tiny', 'base', 'small', 'large'], "vit parameter must be tiny, basem or large"
    if vit == 'base':
        vision_width = 768
        ...
        vision_width = 192
        visual_encoder = VisionTransformer(img_size=image_size, patch_size=16, embed_dim=vision_width, depth=12,
                                           num_heads=3, use_grad_checkpointing=use_grad_checkpointing,
                                           ckpt_layer=ckpt_layer,
                                           drop_path_rate=0 or drop_path_rate
                                           )
